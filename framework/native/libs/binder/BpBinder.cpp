...
namespace android
{
    ...
    BpBinder::BpBinder(int32_t handle, int32_t trackedUid)
        : mStability(0), mHandle(handle), mAlive(1), mObitsSent(0), mObituaries(nullptr), mTrackedUid(trackedUid)
    {
        ...
        wp<DeathRecipient> *outRecipient)
    {
        ...
        for (size_t i = 0; i < N; i++)
        {
            ...
            if ((obit.recipient == recipient || (recipient == nullptr && obit.cookie == cookie)) && obit.flags == flags)
            {
    ...
    void BpBinder::onFirstRef()
    {
        ...
        IPCThreadState *ipc = IPCThreadState::self();
        if (ipc)
            ipc->incStrongHandle(mHandle, this);
    }
