...
namespace android
{
    ...
    status_t Parcel::appendFrom(const Parcel *parcel, size_t offset, size_t len)
    {
        ...
        if ((offset > parcel->mDataSize) || (len > parcel->mDataSize) || (offset + len > parcel->mDataSize))
        {
            return BAD_VALUE;
        }
        ...
        if (numObjects > 0)
        {
            ...
            {
                if ((size_t)numObjects > SIZE_MAX - mObjectsSize)
                    return NO_MEMORY; // overflow
                if (mObjectsSize + numObjects > SIZE_MAX / 3)
                    return NO_MEMORY; // overflow
    ...
    status_t Parcel::writeInterfaceToken(const char16_t *str, size_t len)
    {
        ...
        writeInt32(threadState->shouldPropagateWorkSource() ? threadState->getCallingWorkSourceUid() : IPCThreadState::kUnsetWorkSource);
        writeInt32(kHeader);
    ...
    status_t Parcel::writeObject(const flat_binder_object &val, bool nullMetaData)
    {
        ...
        if (!enoughObjects)
        {
            if (mObjectsSize > SIZE_MAX - 2)
                return NO_MEMORY; // overflow
            if ((mObjectsSize + 2) > SIZE_MAX / 3)
                return NO_MEMORY; // overflow
    ...
    {
        size_t size;
        if (status_t status = reserveOutVector(val, &size); status != OK)
            return status;
                                     ...
                                     const binder_size_t *objects, size_t objectsCount, release_func relFunc)
    {
        ...
        for (size_t i = 0; i < mObjectsSize; i++)
        {
            ...
            const flat_binder_object *flat = reinterpret_cast<const flat_binder_object *>(mData + offset);
    ...
    void Parcel::print(TextOutput &to, uint32_t /*flags*/) const
    {
        ...
        else if (dataSize() > 0)
        {
            ...
            {
                const flat_binder_object *flat = reinterpret_cast<const flat_binder_object *>(DATA + OBJS[i]);
                to << endl
    ...
    status_t Parcel::continueWrite(size_t desired)
    {
        ...
        else
        {
            ...
            if (!(mDataCapacity == 0 && mObjects == nullptr && mObjectsCapacity == 0))
            {
    ...
    void Parcel::scanForFds() const
    {
        ...
        for (size_t i = 0; i < mObjectsSize; i++)
        {
            const flat_binder_object *flat = reinterpret_cast<const flat_binder_object *>(mData + mObjects[i]);
            if (flat->hdr.type == BINDER_TYPE_FD)
            {
                hasFds = true;
                break;
    ...
    Parcel::Blob::Blob() : mFd(-1), mData(nullptr), mSize(0), mMutable(false)
    {
    }
